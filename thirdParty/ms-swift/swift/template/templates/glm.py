...
class GLM4_5VTemplate(GLM4vPackingTemplateMixin, GLM4_5Template):
    ...
    def init_processor(self, processor) -> None:
        super().init_processor(processor)
        if not getattr(GLM4_5VTemplate, '_patched', False):
            GLM4_5VTemplate._patched = True
            from transformers.models.glm4v import modeling_glm4v
            self._patch_create_causal_mask(modeling_glm4v)
            
            # 修复视觉编码器 position_ids 在 CPU 导致 flash attention 报错的问题
            _old_vision_forward = modeling_glm4v.Glm4vVisionModel.forward
            def _new_vision_forward(self, pixel_values, grid_thw=None, **kwargs):
                if grid_thw is not None and pixel_values is not None:
                    grid_thw = grid_thw.to(pixel_values.device)
                return _old_vision_forward(self, pixel_values, grid_thw=grid_thw, **kwargs)
            modeling_glm4v.Glm4vVisionModel.forward = _new_vision_forward
