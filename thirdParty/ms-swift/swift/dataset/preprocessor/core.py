...
class RowPreprocessor:
    ...
    def _check_objects(row):
        ...
        # check bbox
        for box in bbox:
            assert len(box) in {2, 4, 6}, f'len(box): {len(box)}'
            if len(box) == 2:
                continue
            if len(box) == 6:
                # 3D: [x1,y1,z1, x2,y2,z2]
                if box[0] > box[3]: box[0], box[3] = box[3], box[0]
                if box[1] > box[4]: box[1], box[4] = box[4], box[1]
                if box[2] > box[5]: box[2], box[5] = box[5], box[2]
            else:
                # 2D: [x1,y1, x2,y2]
