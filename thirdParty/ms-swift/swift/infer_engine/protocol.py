...
class MultiModalRequestMixin:
    ...
    videos: List[str] = field(default_factory=list)
    objects: Dict[str, Any] = field(default_factory=dict)
    skip_base64_extensions: ClassVar[Optional[tuple]] = None
    ...
    def to_base64(mm_data: Union[str, Image.Image, bytes]) -> str:
        ...
        if isinstance(mm_data, str) and not os.path.isfile(mm_data):
            ...
            return mm_data
        # 跳过医学影像格式，直接返回路径
        if isinstance(mm_data, str) and MultiModalRequestMixin.skip_base64_extensions and any(
                mm_data.lower().endswith(ext) for ext in MultiModalRequestMixin.skip_base64_extensions):
            return mm_data
        ...
        for message in self.messages:
            ...
            for item in content:
                ...
                if isinstance(value, str) and (value.startswith('data:') or value.startswith('http')
                    ...
                    continue
                ################################# 跳过医学影像格式 #################################
                skip_exts = MultiModalRequestMixin.skip_base64_extensions
                if isinstance(value, str) and skip_exts and any(value.lower().endswith(ext) for ext in skip_exts):
                    continue
                #################################################################################
